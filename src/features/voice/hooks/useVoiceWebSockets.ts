...
/** A tool call whose execution is deferred until response.done confirms the response wasn't cancelled. */
interface DeferredToolCall {
  callId: string;
  toolName: string;
  /** Accumulated JSON arguments string (from function_call_arguments.delta events). */
  argsStr: string;
}

/** Per-response tracking for parallel tool calls (Issue 4). */
interface PendingResponse {
  callIds: Set<string>;
  done: boolean;
  hadToolCalls: boolean;
  /** Tool calls waiting to execute until response.done tells us the response wasn't cancelled. */
...
export function useVoiceWebSockets(
  ...
  /** Stored so we can resume recording after a paused elicitation. */
  ...
  /** true when recorder is intentionally paused for an elicitation form */
  const audioPausedRef = useRef(false);

  /** Pending parallel tool call tracking (Issue 4) */
  const pendingResponsesRef = useRef<Map<string, PendingResponse>>(new Map());

  /** Accumulated function-call argument strings keyed by item_id (from response.function_call_arguments.delta). */
  const argAccumRef = useRef<Map<string, string>>(new Map());

  /** Pending post-tool response.create invocations waiting for any active response to finish. */
  const pendingPostToolFiresRef = useRef<number>(0);

  /** Tools ref so they can be updated mid-session (Issue 3) */
  const toolsRef = useRef<Tool[] | undefined>(undefined);

  /** ToolContext factory ref so VoiceProvider can update it mid-session */
  ...
  // ── Parallel-response helper (Issue 4) ───────────────────────────────────

  /** True if any response other than `excludeId` is still being generated by the server. */
    ...
    (responseId: string, ws: WebSocket) => {
      ...
      if (entry.done && entry.callIds.size === 0 && entry.hadToolCalls) {
        ...
        if (hasOtherActiveResponse()) {
          // Another response (e.g. user interjection answered by VAD) is still being generated.
          // Defer firing response.create until that completes to avoid
  ...
  /** Drain any deferred post-tool response.create now that nothing else is in flight. */
  ...
  const start = async (
    ...
    try {
      ...
      // Issue 5: start recording only after session.updated; safety fallback after 3 s
      ...
      ws.addEventListener("message", async (e) => {
        ...
        switch (msg.type) {
          // Issue 5: start recording only after session is applied
          ...
          case "response.created": {
            ...
            // Issue 4: track pending responses
          ...
          case "response.function_call_arguments.delta": {
            // Accumulate streamed argument deltas so we always have the complete args string.
          ...
          case "response.output_item.done": {
            ...
            if (item?.type === "function_call") {
              ...
              // Use accumulated args from deltas — more reliable than item.arguments which can be
              // truncated when a response is cancelled mid-stream.
          ...
          case "response.done": {
            ...
            // Only emit assistant text for completed (not cancelled) responses.
            ...
            if (doneResponseId) {
              ...
              if (entry) {
                ...
                if (responseStatus === "cancelled" && deferredCalls.length > 0) {
                  // Response was cancelled (user interrupted) — discard all pending tool calls.
                  // The args are likely truncated, and the conversation item is incomplete.
                  ...
                  // Response completed normally — execute all deferred tool calls.
                  entry.done = true;

                  if (deferredCalls.length > 0) {
                    for (const deferred of deferredCalls) {
                      // Each call runs concurrently; each decrements callIds and may fire response.create.
                      void (async () => {
                        ...
                        // Pre-flight: parse args and resolve tool before executing.
                        ...
                        if (args === undefined) {
                          ...
                          try {
                            ...
                            // Issue 9: serializeToolResultForApi replaces binary content with text stubs
                        ...
                        if (e) {
                          ...
                          // Entry was lost (e.g. session reset while tool was running).
                          console.warn(
                    ...
                    // No tool calls — just mark done and check if response.create needed.
                    checkAndFireResponseCreate(doneResponseId, eventWs);
                    // No more tool calls in this entry — drop it so it doesn't keep
                    // hasOtherActiveResponse() returning stale results.
                    pendingResponsesRef.current.delete(doneResponseId);
                  }
                }
              }
            }
            // A response just finished — if any tool-completion response.create was deferred
            // because this response was active, fire it now.
  ...
  const updateSession = useCallback(
    (tools?: Tool[], instructions?: string, toolContextFactory?: ToolContextFactory) => {
      ...
      // Send incremental update — always include session.type so the API accepts the event
...
// ── Module-level audio/encoding helpers ─────────────────────────────────────
