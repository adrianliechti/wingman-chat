...
export function WorkflowNode({
  ...
  return (
    ...
    >
      <NodeResizer minWidth={minWidth} minHeight={minHeight} isVisible={selected} lineStyle={{ borderColor: "transparent", borderWidth: 10 }} handleStyle={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: "transparent" }} />

      {/* Input Handle */}
      {showInputHandle && <Handle type="target" position={Position.Left} className="w-3! h-3! bg-green-500! border-2! border-white! dark:border-gray-800!" />}

      {/* Output Handle */}
      {showOutputHandle && <Handle type="source" position={Position.Right} className="w-3! h-3! bg-blue-500! border-2! border-white! dark:border-gray-800!" />}
      ...
      <div className="flex items-center justify-between mb-3 shrink-0">
        ...
        <div className="flex items-center gap-2">
          {headerActions}
          <button onClick={() => deleteNode(id)} className="text-gray-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400 transition-colors">
            <Trash2 size={14} />
          </button>
          {onExecute && (
            <button onClick={onExecute} disabled={isProcessing || !canExecute} className={`${styles.button} transition-colors disabled:opacity-50`} title="Execute">
