    ...
    return (
      <div className="h-full flex items-center justify-center p-8">
        <div className="text-center max-w-sm">
          ...
          <p className="text-sm text-neutral-500 mb-6">{percentage >= 80 ? "Excellent work!" : percentage >= 60 ? "Good effort!" : "Keep studying!"}</p>
          <button type="button" onClick={handleRestart} className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-neutral-800 dark:bg-neutral-200 text-white dark:text-neutral-900 rounded-lg hover:opacity-90 transition-opacity">
            <RotateCcw size={14} />
            Try again
          </button>
        </div>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 pt-6 pb-2 shrink-0">
        ...
        <div className="h-1 bg-neutral-200 dark:bg-neutral-800 rounded-full overflow-hidden">
          <div className="h-full bg-neutral-800 dark:bg-neutral-200 rounded-full transition-all duration-300" style={{ width: `${((currentIndex + (revealed ? 1 : 0)) / questions.length) * 100}%` }} />
        </div>
      </div>

      {/* Question */}
      <div className="flex-1 overflow-y-auto px-6 py-4 min-h-0">
        <h3 className="text-base font-medium text-neutral-800 dark:text-neutral-200 mb-5 leading-relaxed">{question.question}</h3>
        ...
        <div className="space-y-2.5">
          {question.options.map((option, i) => {
            ...
            return (
              <button key={i} type="button" onClick={() => handleSelect(i)} disabled={revealed} className={`w-full text-left px-4 py-3 rounded-lg border-2 transition-all ${styles}`}>
                <div className="flex items-center gap-3">
                  <span className="shrink-0 w-6 h-6 rounded-full border border-neutral-300 dark:border-neutral-600 flex items-center justify-center text-xs font-medium text-neutral-500">
                    {revealed && isCorrect ? <Check size={14} className="text-green-600" /> : revealed && isSelected && !isCorrect ? <X size={14} className="text-red-600" /> : String.fromCharCode(65 + i)}
                  </span>
