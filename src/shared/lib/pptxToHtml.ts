...
async function renderGraphicFrame(frame: Element, part: PartCtx, ctx: SlideCtx, opts: TreeOpts): Promise<string> {
  ...
  if (uri.includes("/chart")) {
    ...
    if (rel && !rel.external) {
      ...
      if (chartDoc) {
        const data = parseChart(chartDoc, { theme: ctx.theme, clrMap: ctx.clrMap }, ctx.theme);
        if (data?.series.length) return renderChart(data, xfrm, opts);
      }
    }
  }
...
function parseChart(doc: Document, env: ColorEnv, theme: Theme): ChartData | null {
  ...
  const accents = ["accent1", "accent2", "accent3", "accent4", "accent5", "accent6"].map((a) =>
    theme.colors[a] ? `#${theme.colors[a]}` : undefined,
  );
  ...
  els(typeEl, "ser").forEach((ser, si) => {
    ...
    const fillEl = findFillElement(child(ser, "c:spPr"));
    const color = (fillEl ? resolveColorIn(fillEl, env) : undefined) || accents[si % 6] || "#4472C4";
    ...
    for (const dPt of els(ser, "dPt")) {
      ...
      const f = findFillElement(child(dPt, "c:spPr"));
      ptColors[idx] = f ? resolveColorIn(f, env) : undefined;
    }
...
function renderChart(data: ChartData, xfrm: Xfrm, opts: TreeOpts): string {
  const W = Math.max(xfrm.w, 60);
  const H = Math.max(xfrm.h, 40);
  ...
  // Legend band (bottom): pie → categories, else → series names
  ...
  if (isPie) {
    svg.push(renderPie(data, W, top, bottom));
  } else {
    svg.push(renderCartesian(data, W, top, bottom, stacked));
  }

  return (
    `<div style="${positionStyle(xfrm, opts)}background:#fff;overflow:hidden;">` +
    `<svg width="100%" height="100%" viewBox="0 0 ${Math.round(W)} ${Math.round(H)}" preserveAspectRatio="none" font-family="inherit">${svg.join("")}</svg>` +
    `</div>`
  );
}
...
function renderCartesian(data: ChartData, W: number, top: number, bottom: number, stacked: boolean): string {
  ...
  const out: string[] = [];
  // Gridlines + value labels (value axis)
  const ticks = 4;
