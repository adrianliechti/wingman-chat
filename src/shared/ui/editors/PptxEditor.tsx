...
export const PptxEditor = memo(function PptxEditor({ path, content, contentType }: PptxEditorProps) {
  const [result, setResult] = useState<PptxHtmlResult | null>(null);
  ...
  useEffect(() => {
    ...
    pptxToHtml(file)
      ...
      .catch((e) => {
        console.error("PPTX slide preview failed, falling back to text preview:", e);
        ...
        <div className="h-full flex items-center justify-center px-3 pt-2 pb-8" ref={setSlideContainer}>
          ...
          >
            <iframe
              ...
              // allow-scripts: slide documents carry a small inline autofit
              // script (see pptxToHtml) that shrinks overflowing text bodies
              // the way PowerPoint does. The HTML is generated by us from
              // escaped deck content, with no external resources.
